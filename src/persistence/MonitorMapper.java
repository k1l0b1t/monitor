...
public class MonitorMapper
{
	...
	private void loadConfigurationFile()
	{
		...
		this.connectionString = String.format("jdbc:mysql://%s:%s@%s/%s", user,
	...
	public List<Monitor> listMonitors() throws UnknownHostException
	{
		...
		try (Connection conn = DriverManager.getConnection(connectionString);
			PreparedStatement query = conn.prepareStatement(
				"select monitor.name, monitor.ip from monitor"))
		{
			...
			try (ResultSet rs = query.executeQuery())
			{
				...
				{
					monitors.add(
						new Monitor(rs.getString("name"), rs.getString("ip")));
				}
			}
	...
	public void updateMonitor(String name, boolean status)
	{
		...
		try (Connection conn = DriverManager.getConnection(connectionString);
			PreparedStatement query = conn
				.prepareStatement("update monitor set status=" + status
					+ " where name='" + name + "'"))
		{
			query.executeUpdate();
		} catch (SQLException ex)
